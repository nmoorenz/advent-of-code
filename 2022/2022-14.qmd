...
cave = np.chararray((200, 600))
cave[:] = '.'

for r in rk:
    for d in range(len(r) - 1):
        e,f = (int(x) for x in r[d].split(','))
        g,h = (int(x) for x in r[d+1].split(','))
        if e == g:
...
[row.tobytes() for row in cave[15:170,450:540]]
